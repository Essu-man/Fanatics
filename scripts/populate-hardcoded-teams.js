    ...
    try {
        ...
        if (existingTeams.size > 0) {
            ...
            for (const doc of existingTeams.docs) {
                ...
                if (!data.sport) {
                    // Determine sport based on league or team name
                    const league = data.league || "";
                    updates.sport = [
                        "NBA", "Premier League (Basketball)"
                    ].some(l => league.includes(l)) ? "basketball" : "football";
                }
                if (data.enabled === undefined) {
                    updates.enabled = false;
                }
                if (data.isHardcoded === undefined) {
                ...
                if (Object.keys(updates).length > 0) {
                    ...
                    updatedCount++;
                    console.log(`Updated ${data.name || doc.id} with fields: ${Object.keys(updates).join(", ")}`);
                }
            }

            if (updatedCount > 0) {
                console.log(`\n✅ Updated ${updatedCount} teams with missing fields.`);
            } else {
